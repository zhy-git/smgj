...
class Mssc extends Home
{
    public function index(){
        $number = getNumberCache('mssc');
        ...
        //获取公共赔率
        $oddsList = Db::name('odds')->where('cate',8)->cache(10)->select();
        //获取历史开奖数据
        ...
        $openList = Db::name('mssc_stage')->where($whereTime)->order('id DESC')->select();
        //获取当前最新注单
        //计算下期的数字
        $whereData['end_time'] = array('gt',time());
        $stage = Db::name('mssc_stage')->where($whereData)->find();
        $whereSingle['s.uid'] =  $this->uid;
        $whereSingle['s.cate'] = 8;
...
//        }
        $time = time();
        $whereData['end_time'] = array('gt',$time);
        $stageData = Db::name('mssc_stage')->where($whereData)->find();
...
//        print_r($insertData);
        foreach ($insertData as $inl=>$idv) {
            ...
            try {
                ...
                $returnInsertData[$inl]['typeName'] = $typeNameArr[8][$idv['type']];
                ...
                $explain = $insertData[$inl]['stage'].'期 '.$typeNameArr[8][$idv['type']].'：'.$idv['notes'].'注 '.$idv['multiple'].'倍';
                addDetail($this->uid,2,$idv['money'],$balance,5,8,$explain,1,$returnInsertData[$inl]['id']);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
