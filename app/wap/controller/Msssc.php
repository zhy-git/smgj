...
class Msssc extends Home
{
    public function index(){
        ...
        //获取公共赔率
        $oddsList = Db::name('odds')->where('cate',7)->cache(10)->select();
...
//        }
        ...
        foreach ($insertData as $inl=>$idv) {
            ...
            try {
                ...
                $returnInsertData[$inl]['typeName'] = $typeNameArr[7][$idv['type']];
                ...
                $explain = $insertData[$inl]['stage'].'期 '.$typeNameArr[7][$idv['type']].'：'.$idv['notes'].'注 '.$idv['multiple'].'倍';
                addDetail($this->uid,2,$idv['money'],$balance,5,7,$explain,1,$returnInsertData[$inl]['id']);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
