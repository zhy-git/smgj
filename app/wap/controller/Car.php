...
class Car extends Home
{
    public function index(){
        ...
        //获取公共赔率
        $oddsList = Db::name('odds')->where('cate',1)->cache(10)->select();
        //获取历史开奖数据
...
//        print_r($insertData);
        foreach ($insertData as $inl=>$idv) {
            ...
            try {
                ...
                $returnInsertData[$inl]['typeName'] = $typeNameArr[1][$idv['type']];
                ...
                addDetail($this->uid,2,$idv['money'],$balance,5,1,$explain,1,$returnInsertData[$inl]['id']);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
