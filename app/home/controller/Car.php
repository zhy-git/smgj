    ...
    public function betting(Request $request){
        ...
        foreach ($insertData as $inl=>$idv) {
            ...
            try {
                ...
                $returnInsertData[$inl]['typeName'] = $typeNameArr[$sdv['cate']][$idv['type']];
                ...
                $explain = $insertData[$inl]['stage'].'期 '.$typeNameArr[$sdv['cate']][$idv['type']].'：'.$idv['notes'].'注 '.$idv['multiple'].'倍';
                addDetail($this->uid,2,$idv['money'],$balance,5,$sdv['cate'],$explain,1,$returnInsertData[$inl]['id']);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
