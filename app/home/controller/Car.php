...
class Car extends Home
{
    public function index(){
        ...
        $whereTime['number'] = array('neq', '');
        $openList = Db::name('car_stage')->where($whereTime)->order('id DESC')->select();
        ...
        $stage = Db::name('car_stage')->where($whereData)->find();
        ...
        switch ($typePage){
            ...
            default:
                ...
                for($i=1;$i<=10;$i++){
                    $rule_data['number'][$i] = Db::name('odds')->field($field)->where('cate',$cate)->where('type','<=',10)->where('status',1)->where('rule',$i)->order('type')->cache(60)->select();
                }
                $template = 'index';
                break;
        }
    ...
    public function betting(Request $request){
        ...
        $time = time();
        $whereData['end_time'] = array('gt',$time);
        $stageData = Db::name('car_stage')->where($whereData)->find();
        ...
        foreach ($insertData as $inl=>$idv) {
            ...
            try {
                ...
                $explain = getCate($idv['cate'],'name').'-'.getHall($idv['cate'],$idv['hall']).'-'.$insertData[$inl]['stage'].'期 '.$typeNameArr[$sdv['cate']][$idv['type']].'-'.$idv['number'].'：'.$idv['notes'].'注 '.$idv['multiple'].'倍';
                addDetail($this->uid,2,$idv['money'],$balance,5,$sdv['cate'],$explain,$idv['hall'],$returnInsertData[$inl]['id']);
                // 提交事务
                Db::commit();
            } catch (\Exception $e) {
                // 回滚事务
