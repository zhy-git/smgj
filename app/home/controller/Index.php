        ...
        if (request()->isMobile()){
            ...
            if(isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'],'WEBVIEW_APP') !== false){//app访问
                ...
                if(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone') || strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')){
                    $url = 'mqq://im/chat?chat_type=wpa&uin='.$service_qq.'&version=1&src_type=web';
                }else if(strpos($_SERVER['HTTP_USER_AGENT'], 'Android')){
                    $url = 'mqqwpa://im/chat?chat_type=wpa&uin='.$service_qq;
                }else{
                    $url = 'mqqwpa://im/chat?chat_type=wpa&uin='.$service_qq;
                }
            }else{
                ...
                $real[0] = $url[0];
                $real[1] = $url[1];
//                $url = 'http://wpa.qq.com/msgrd?v=3&uin='.$service_qq
                $url = "mqqwpa://im/chat?chat_type=wpa&uin=".$service_qq."&version=1&src_type=web";
            }
//            dump($url);die;
        ...
        $notice = Db::name('notice')->where('title','紧急公告')->order('id desc')->find();
        return view('',[
//            'url'=>$url,
    ...
    public function hall_cl(Request $request){
        $cate = $request->param('cate');
        $field = Db::name('cate')->where('id',$cate)->value('nickname');
        $openList = Db::name('at_'.$field)->order('id DESC')->limit(10)->select();
        if($cate==1 || $cate==2){
            foreach ($openList as $k=>&$v){
                $v['number']=explode(',',$v['number']);
                $v['sum'] = array_sum($v['number']);
                $v['dx'] = dxJudgment(14,$v['sum']);
                $v['ds'] = dsJudgment($v['sum']);
            }
        }elseif ($cate==8){
            foreach ($openList as $k=>&$v){
                $v['number']=explode(',',$v['number']);
