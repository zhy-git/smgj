...
class Xync extends Home
{
    public function index(){
        $number = getNumberCache('xync');
        ...
        $openList = Db::name('xync_stage')->where($whereTime)->order('id DESC')->select();
        //获取当前最新注单
        //计算下期的数字
        $whereData['end_time'] = array('gt',time());
        $stage = Db::name('xync_stage')->where($whereData)->find();
...
//        }
        $time = time();
        $whereData['end_time'] = array('gt',$time);
        $stageData = Db::name('xync_stage')->where($whereData)->find();
        ...
        switch ($type){
            case 1://冠.亚军和
                switch ($number){
                    case '总和大':
                    case '总和小':
                        $returnData['win_id'] = 264;
                        break;
                    case '总和单':
                        $returnData['win_id'] = 268;
                        break;
                    case '总和尾大':
                        $returnData['win_id'] = 270;
                        break;
                    case '总和双':
                        $returnData['win_id'] = 269;
                        break;
                    case '总和尾小':
                        $returnData['win_id'] = 271;
                        break;
                }
                break;
            case 2://冠军
