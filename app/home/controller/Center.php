...
class Center extends Home
{
    public function index(){
        if (request()->isMobile()){
            if(isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'],'WEBVIEW_APP') !== false){//app访问
                ...
                $url = 'http://tb.53kf.com/code/client/10172642/1';
            }
            $this->assign('url',$url);
    ...
    public function back_record(){
        ...
        foreach ($data as $k=>&$v){
            $v['back'] = Db::name('detail')->where('status',2)->where('create_at','>=',strtotime($v['days']))->where('create_at','<',(strtotime($v['days'])+86400))->where('exp',11)->where('uid',$this->uid)->sum('money');
        }
        return view('',[
            'data'=>$data,
        ]);
