...
class Agent extends Home
{
    ...
    public function index(){
        ...
        switch ($typePage){
            case 'add':
                $memDetail = Db::name('member')->where('id',$this->uid)->field('id,nickname,money,ag_money,bb_money,unpaid_money,is_proxy,rebate,back_rebate,bonus')->find();
                if($memDetail){
    ...
    public function groupList(Request $request){
        $memDetail = Db::name('member')->where('id',$this->uid)->field('id,nickname,money,back_rebate,is_proxy,rebate,bonus')->find();
    ...
    public function addDo(Request $request){
        ...
        $checkData['back_rebate'] = $data['back_rebate'];
        if(is_float($data['back_rebate']) || is_int($data['back_rebate'])){
            $this->error("请填写正确返点");
        }
        ...
        $insertData['back_rebate'] = $data['back_rebate'];
//        $insertData['rebate'] = $rebateArr[0];
//        $insertData['bonus'] = $rebateArr[1];
        $member = new Members();
        $userInfo = $member->where('id',$this->uid)->field('id,is_proxy,rebate,bonus,proxy_ids,junior_ids,back_rebate')->find();
        if($data['agent_user']==0 && $data['back_rebate']!=0){
            $this->error('普通会员返点只能为0');
        }
        if($userInfo['is_proxy']==0){
            $this->error('不能开通代理用户');
        }
        if($data['agent_user']!=0 && $userInfo['is_proxy']>=$data['agent_user']){
            $this->error("开户代理等级必须低于自己");
        }
        if($userInfo['back_rebate']<$insertData['back_rebate']){
            $this->error('账号返点不符合规范');
        }
...
//        $insertData['bonus'] = $rebateArr[1];
        ...
        $member = new Members();
        $userInfo = Db::name('member')->where('id',$this->uid)->field('id,is_proxy,rebate,bonus,proxy_ids,junior_ids,back_rebate')->find();
        if($userInfo['is_proxy']==0){
            $this->error('不能开通代理用户');
        }
        if($data['agent_user']!=0 && $userInfo['is_proxy']>=$data['agent_user']){
            $this->error("推广链接用户代理等级必须低于自己");
        }
        if($userInfo['back_rebate']<$insertData['back_rebate']){
            $this->error('账号返点不符合规范');
        }
        ...
        if($res){
            foreach ($res as $k => $v) {
                if(!array_key_exists($v['uid'],$returnData['data'])){
                    ...
                    $returnData['data'][$v['uid']]['self_back'] = 0;
                    $returnData['data'][$v['uid']]['proxy_back'] = 0;
                }
                switch ($v['exp']) {
                    case 1:
                        $returnData['data'][$v['uid']]['charge'] = bcadd($returnData['data'][$v['uid']]['charge'], $v['money'], 2);
                        break;
                    case 2:
                        $returnData['data'][$v['uid']]['win'] = bcadd($returnData['data'][$v['uid']]['win'], $v['money'], 2);
                        break;
                    case 3:
                        $returnData['data'][$v['uid']]['cedan'] = bcadd($returnData['data'][$v['uid']]['cedan'], $v['money'], 2);
                        break;
                    case 4:
                        $returnData['data'][$v['uid']]['cash'] = bcadd($returnData['data'][$v['uid']]['cash'], $v['money'], 2);
                        break;
                    case 5:
                        $returnData['data'][$v['uid']]['bet'] = bcadd($returnData['data'][$v['uid']]['bet'], $v['money'], 2);
                        break;
                    case 6:
                        $returnData['data'][$v['uid']]['back'] = bcadd($returnData['data'][$v['uid']]['back'], $v['money'], 2);
                        break;
                    case 7:
                        $returnData['data'][$v['uid']]['act'] = bcadd($returnData['data'][$v['uid']]['act'], $v['money'], 2);
                        break;
                    case 8:
                        $returnData['data'][$v['uid']]['in'] = bcadd($returnData['data'][$v['uid']]['in'], $v['money'], 2);
                        break;
                    case 9:
                        $returnData['data'][$v['uid']]['out'] = bcadd($returnData['data'][$v['uid']]['out'], $v['money'], 2);
                        break;
                    case 10:
                        $returnData['data'][$v['uid']]['other'] = bcadd($returnData['data'][$v['uid']]['other'], $v['money'], 2);
                        break;
                    case 11:
                        $returnData['data'][$v['uid']]['self_back'] = bcadd($returnData['data'][$v['uid']]['self_back'], $v['money'], 2);
                        break;
                    case 12:
                        $returnData['data'][$v['uid']]['proxy_back'] = bcadd($returnData['data'][$v['uid']]['proxy_back'], $v['money'], 2);
                        break;
                }
            }
//            foreach ($returnData['data'] as $k=>&$v){
//                $room_bet = Db::name('detail')->alias('d')->where($whereData)->where('uid',$k)->where('exp',5)->where('cate',2)->where('hall','>',1)->where('status',2)->sum('money');
//                $room_z = Db::name('detail')->alias('d')->where($whereData)->where('uid',$k)->where('exp',2)->where('cate',2)->where('hall','>',1)->where('status',2)->sum('money');
//                $room_ce = Db::name('detail')->alias('d')->where($whereData)->where('uid',$k)->where('exp',3)->where('cate',2)->where('hall','>',1)->where('status',2)->sum('money');
//                $v['bet']   = bcsub($v['bet'],$room_bet,2);
//                $v['cedan'] = bcsub($v['cedan'],$room_ce,2);
//                $v['win']   = bcsub($v['win'],$room_z,2);
//            }
            return json($returnData);
        }else{
            $returnData['code'] = 0;
...
//        }
    ...
    public function getTeam(Request $request){
        ...
        if($data['username']){
            ...
            $mem = Db::name('member')->where('gm_name',$data['username'])->field('id,gm_name,rebate,bonus,proxy_id,proxy_ids,junior_ids,junior_num,money,is_proxy,back_rebate,is_tmoney')->select();
            if($mem){
                $proxyIdsArr = explode(',',$mem[0]['proxy_ids']);
                if(array_search($this->uid,$proxyIdsArr) === false){
                    $this->error('暂无数据');
            ...
            $mem = Db::name('member')->where('id',$this->uid)->field('id,gm_name,rebate,proxy_id,proxy_ids,junior_ids,back_rebate,junior_num')->find();
            $whereData['id'] = array('in',$mem['junior_ids']);
            $res = Db::name('member')->where($whereData)->field('id,gm_name,rebate,bonus,proxy_id,proxy_ids,junior_ids,junior_num,money,is_proxy,back_rebate,is_tmoney')->select();
            if($res){
                $login_array = cache('login_array');
                foreach ($res as $k=>$v){
                    if (isset($login_array['uid_'.$v['id']])){
                    ...
                    if($v['junior_ids']){
                        $uids = explode(',',$v['junior_ids']);
                        array_push($uids,$v['id']);
                        $whereData2['id'] = array('in',$uids);
    ...
    public function getNextTeam(Request $request){
        ...
        $whereData['id'] = array('in',$mem['junior_ids']);
        $res = Db::name('member')->where($whereData)->field('id,gm_name,rebate,bonus,proxy_ids,junior_ids,junior_num,money,back_rebate,is_proxy')->select();
        if($res){
            $login_array = cache('login_array');
            foreach ($res as $k=>$v){
                if($v['junior_ids']){
    ...
    public function teamDetail(Request $request){
        ...
        $mem = Db::name('member')->where('id',$data['id'])->field('id,gm_name,rebate,bonus,proxy_ids,junior_ids,junior_num,money,is_proxy,create_at,back_rebate,update_at')->find();
        if($mem){
            //在线人数
            $mem['onlineNum'] = 0;
            if($mem['junior_ids']){
                ...
                $mem['teamMoney'] = bcadd($mem['teamMoney'],$mem['money'],2);
            }else{
                $mem['teamMoney'] = bcadd(0,$mem['money'],2);
            }
            //当前代理级别
            if($mem['is_proxy'] == 1){
                $mem['is_proxy'] = '总代理';
            ...
            if(!empty($data['rangeTime'])){
                $rangeTime = explode('~',$data['rangeTime']);
                $start = strtotime($rangeTime[0]);
                $end = strtotime($rangeTime[1])+86400;
                if($start<$thirtyDaysAgo){
                    $start = $thirtyDaysAgo;
                }
            }else{
            ...
            //团队uid
            if(!empty($mem['junior_ids'])){
                $uids = explode(',',$mem['junior_ids']);
            }else{
                $uids=[];
            }
            array_push($uids,$mem['id']);
            $detailWhereData['uid'] = array('in',$uids);
            $detailWhereData['create_at'][] = array('>=',$start);
            $detailWhereData['create_at'][] = array('<=',$end);
            $detailWhereData['is_room'] = 0;
            ...
            $mem['teamBet'] = bcsub($mem['teamBet'],$mem['teamCe'],2);
            $mem['teamYk'] = bcadd(bcsub($mem['teamWin'] ,$mem['teamBet'],2) , $mem['teamFan'],2);
...
//                Db::name('member')->where($whereData)->field('id,gm_name,rebate,proxy_ids,junior_ids,junior_num')->find();
    ...
    public function ransferMoney(Request $request){
        ...
        $self_money = Db::name('member')->where('id',$this->uid)->find();
        if($self_money['is_tmoney']==0){
            return json(array('code'=>0,'msg'=>'你无此权限！请联系客服'));
        }
        if($self_money['money']<$data['money']){
    ...
    public function team_page(Request $request){
        ...
        $mem = Db::name('member')->where('id',$this->uid)->field('id,gm_name,rebate,proxy_id,proxy_ids,junior_ids,back_rebate,junior_num')->find();
        $whereData['id'] = array('in',$mem['junior_ids']);
        $res = Db::name('member')->where($whereData)->field('id,gm_name,rebate,bonus,proxy_id,proxy_ids,junior_ids,back_rebate,junior_num,money,is_proxy,is_tmoney')->select();
        if($res){
            foreach ($res as $k=>&$v){
                $v['flow'] = Db::name('single')->where('uid',$v['id'])->where($w)->where('is_del',0)->where('state',1)->where('is_room',0)->sum('money');
                $v['z_money'] = Db::name('single')->where('uid',$v['id'])->where($w)->where('is_del',0)->where('state',1)->where('is_room',0)->sum('z_money');
            }
        }
    ...
    public function agent_back(){
        $junior_ids = Db::name('member')->where('id',$this->uid)->value('junior_ids');
        $data = Db::name('single')
            ->field("FROM_UNIXTIME(create_at,'%Y%m%d') days,sum(money) bet_money")
            ->where('uid','in',$junior_ids)
            ->where('is_del',0)
            ->group("days")
            ->order('days desc')
            ->select();
        foreach ($data as $k=>&$v){
            $v['back'] = Db::name('detail')
                ->where('create_at','>',(strtotime($v['days'])+3600))
                ->where('create_at','<',(strtotime($v['days'])+90000))
                ->where('status',2)
                ->where('exp',12)
                ->where('uid',$this->uid)
                ->sum('money');
