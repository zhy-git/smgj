...
//    	    $this->assign('users',['uid'=>$this->uid,'name'=>$member->gm_name,'mobile'=>$member->mobile,'head'=>$member->head,'money'=>$umoney,'rebate'=>$member->rebate]);
            ...
            if ($memInfo['session_id'] != session_id()){
...
//                $this->redirect('Login/index');
            ...
            if($memInfo['m_status']==1){
                session('member_info',null);
                echo "<script type='text/javascript'>alert('您的账号已被管理员冻结，请联系客服');window.location.href='/Home/Login/index'</script>";exit;
            }
