...
class Api
{
    public function getNewOpenList(){
        $cate = input('get.cate');
        if($cate){
            try{
                $list = Db::name('at_'.$cate)->limit(1,10)->order('create_time desc')->select();
                return json(array('code'=>0,'msg'=>'获取成功','data'=>$list));
            }catch (\Exception $e) {
                return json(array('code'=>10005,'msg'=>$e->getMessage()));
            }
        }else{
            return json(array('code'=>10000,'msg'=>'无权限'));
        }
    }
    //检查是否开奖
