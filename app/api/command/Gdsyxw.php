...
class Gdsyxw extends Command
{
    protected function configure()
    {
        $this->setName('gdsyxw')->setDescription('广东11选5');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/gdsyxw_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_gdsyxw a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        switch ($v['type']) {
                            case 1://总和
                                $returnData = gdsyxwZh($v['number'], $number_arr);
                                break;
                            case 2://第一球
                                $returnData = gdsyxwBall($v['number'], $number_arr, $v['type']);
                                break;
                            case 3://第二球
                                $returnData = gdsyxwBall($v['number'], $number_arr, $v['type']);
                                break;
                            ...
                            case 6://第五球
                                ...
                                break;
                            case 7://一中一
                                $returnData = gdsyxwYzy($v['number'], $number_arr);
                                break;
                        }
                        if ($returnData['win']) {
                            $explain = '广东11选5' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //当前玩家每注奖金
                            $perMoney = $v['odds'];
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '广东11选5 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/gdsyxw_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
