...
class Msssc extends Command
{
    ...
    {
        $this->setName('msssc')->setDescription('秒速时时彩兑奖');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/msssc_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_msssc a', 's.stage=a.stage')
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        if ($returnData['win']) {
                            ...
                            $explain = '秒速时时彩' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //计算当前玩法当前玩家每注奖金
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '秒速时时彩 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/msssc_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
