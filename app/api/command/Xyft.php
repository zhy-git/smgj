...
class Xyft extends Command
{
    ...
    {
        $this->setName('xyft')->setDescription('幸运飞艇');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/xyft_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_xyft a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        if ($returnData['win']) {
                            $explain = '幸运飞艇' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //当前玩家每注奖金
                            $perMoney = $v['odds'];
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '幸运飞艇 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/xyft_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
