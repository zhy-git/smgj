...
class Pcdd extends Command
{
    protected function configure()
    {
        $this->setName('pcdd')->setDescription('PC蛋蛋兑奖');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/pcdd_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_pcdd a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        if ($returnData['win']) {
                            $explain = 'PC蛋蛋' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //当前玩家每注奖金
                            $perMoney = $v['odds'];
...
//                                }
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = 'PC蛋蛋 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
...
//                                }
                        ...
                        file_put_contents(__DIR__ . '/pcdd_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
