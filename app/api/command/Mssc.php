...
class Mssc extends Command
{
    protected function configure()
    {
        $this->setName('mssc')->setDescription('秒速赛车');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/mssc_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_mssc a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        if ($returnData['win']) {
                            $explain = '秒速赛车' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //当前玩家每注奖金
                            $perMoney = $v['odds'];
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '秒速赛车 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/mssc_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
