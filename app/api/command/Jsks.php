...
class Jsks extends Command
{
    protected function configure()
    {
        $this->setName('jsks')->setDescription('江苏快三兑奖');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/jsks_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_jsks a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        switch ($v['type']) {
                            case 1://三军、大小
                                $returnData = jsksSjdx($v['number'], $number_arr);
                                break;
                            case 2://围骰、全骰
                                $returnData = jsksWsqs($v['number'], $number_arr);
                                break;
                            case 3://点数
                                $returnData = jsksDs($v['number'], $number_arr);
                                break;
                            case 4://长牌
                                $returnData = jsksCp($v['number'], $number_arr);
                                break;
                            case 5://短牌
                                $returnData = jsksDp($v['number'], $number_arr, $v['type']);
                                break;
                        }
                        if ($returnData['win']) {
                            $explain = '江苏快三' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //当前玩家每注奖金
                            $perMoney = $v['odds'];
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '江苏快3 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/jsks_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
