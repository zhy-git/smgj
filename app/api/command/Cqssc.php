...
class Cqssc extends Command
{
    ...
    {
        $this->setName('cqssc')->setDescription('重庆时时彩兑奖');
    }
    ...
    public function doCron(){
        $fileName = __DIR__.'/cqssc_lock.txt';
        $fp = fopen($fileName, "r");
        if(flock($fp,LOCK_EX | LOCK_NB)) {
            ...
            $list = Db::name('single')
                ...
                ->join('dl_at_cqssc a', 's.stage=a.stage')
            ...
            if ($list) {
                ...
                foreach ($list as $v) {
                    try {
                        ...
                        switch ($v['type']) {
                            case 1://总和-龙虎和
                                $returnData = cqsscZhlhh($v['number'], $number_arr);
                                break;
                            case 2://第一球
                                $returnData = cqsscBall($v['number'], $number_arr, $v['type']);
                                break;
                            case 3://第二球
                                $returnData = cqsscBall($v['number'], $number_arr, $v['type']);
                                break;
                            ...
                            case 6://第五球
                                ...
                                break;
                            case 7://前三
                                $returnData = cqsscQzhsan($v['number'], $number_arr, $v['type']);
                                break;
                            case 8://中三
                                $returnData = cqsscQzhsan($v['number'], $number_arr, $v['type']);
                                break;
                        ...
                        if ($returnData['win']) {
                            ...
                            $explain = '重庆时时彩' . $v['title'] . $v['stage'] . '期，中奖' . $returnData['win'] . '注';
                            //计算当前玩法当前玩家每注奖金
                            //$perMoney = getBonus($pl, $v);
                            $perMoney = $v['odds'];
                            ...
                            if ($isSuccess) {
                                ...
                                $pushInsertData['content'] = '重庆时时彩 第' . $v['stage'] . '期 ' . $v['title'] . ' ' . $v['number'] . ' 已中奖,中奖金额<span>' . $reMoney . '</span>';
                        ...
                        file_put_contents(__DIR__ . '/cqssc_open.txt', $e . $v['id'] . PHP_EOL, FILE_APPEND);
