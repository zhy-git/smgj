...
//            ->select();
    ...
    public function three_four(Request $request){
        if($request->isPost()){
            $cate = $request->param('cate');
            $hall = $request->param('hall');
            $bet_money = $request->param('bet_money');
            $odds_one = $request->param('odds_one');
            $odds_two = $request->param('odds_two');
            $odds_three = $request->param('odds_three');
            $odds_four = $request->param('odds_four');
            $odds_five = $request->param('odds_five');
            if($cate && $hall && $bet_money && $odds_one && $odds_two && $odds_four && $odds_five){
                $map = ['cate'=>$cate,'hall'=>$hall];
                $data=['bet_money'=>$bet_money,'odds_one'=>$odds_one,'odds_two'=>$odds_two,'odds_four'=>$odds_four,'odds_five'=>$odds_five];
                $result=Db::name('three_four')->where($map)->update($data);
                if ($result) {
                    $this->success_new('编辑成功',url('Admin/Lottery/index'));
                }else{
                    $this->error_new('编辑失败');
                }
            }else{
            ...
            $cate = $request->param('cate');
            $hall = $request->param('hall');
            $setting = Db::name('three_four')->where('cate',$cate)->where('hall',$hall)->find();
            if($cate && $hall){
                return view('',[
                    'odds_set'=>$setting,
                ]);
...
//    }
    ...
    public function choose_table($cate){
        $name = Db::name('cate')->where('id',$cate)->value('nickname');
        return $name;
    }
