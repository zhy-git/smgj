...
class admin extends Controller
{
    public function _initialize()
    {
        ...
        $old_session = Db::name('user')->where('id',session('info.id'))->value('session_id');
        if ($old_session != session_id()){
            session('info',null);
            echo "<script type='text/javascript'>alert('您的账号已在别处登陆,若非本人操作，请尽快修改密码');window.location.href='/Admin/Login/index'</script>";exit;
        }

        //判断权限
        $auth=new \Auth\Auth() or die('加载auth类失败');
        $rule_name=$moduleName.'/'.$controllerName.'/'.$actionName;
        if($actionName=='getnewmessage'){
