...
//    }
    ...
    public function addmoney(Request $request){
        ...
        $money    = $request->post('money');
        $remark   = $request->post('remark');
        if(empty($exp) || empty($gm_name) || empty($money)){
            $this->error_new('缺少参数');
        }
        $Members = new Members();
        $member_info = Db::name('member')->where('gm_name',$gm_name)->find();
        if(empty($member_info)){
            $this->error_new('用户不存在');
        }
        Db::startTrans();
        try {
            if($exp==4 || $exp==5 || $exp==9){
                $type = 2;
                $up_data["money"] = array("exp", "money-" . $money);
            }else{
                $type = 1;
            ...
            $result0 = Db::name('member')->where(array('id'=>$member_info['id']))->update($up_data);
            if($exp==1){
                $Recharge = new Recharge();
                ...
                $data['create_at'] = time();
                $result1 = $Recharge->addData($data);
                $this->add_charge_detail($member_info['id'],$money,1);
                $content = "恭喜您，本次后台成功充值".$money."元";
                add_member_msg($member_info['id'],$content);
                //Db::name('member')->where(array('id'=>$member_info['id']))->setInc('score',$money);
                //$score = Db::name('member')->where(array('id'=> $member_info['id']))->value('score');
                //$level = Db::name('classs')->where('score','<=',$score)->order('score desc')->find();
                //Db::name('member')->where('id',$member_info['id'])->update(['level'=>$level['id']]);
            }elseif($exp==4){
                $my_money = $member_info['money'];
                if($my_money<$money){
                    $this->error_new('余额不足');
                }
                ...
                $data['remark'] = $remark;
                $data['create_at'] = time();
                Db::name('withdrawals')->insert($data);
                $explain = '后台提现';
                ...
                $explain = '后台'.getExp($exp);
                $new_money = bcadd(round($member_info['money'],2), round($money,2));
                if(!empty($explain)){
                    $result0 = Db::name('member')->where(array('id'=>$member_info['id']))->update(array('money'=>$new_money));
                    $save['uid']     = $member_info['id'];
                    $save['type']    = $type;
                    $save['money']   = $money;
                    $save['balance'] = $new_money;
            ...
            Db::commit();
            $this->success_new('操作成功',url('admin/money/transaction'));
        }catch (\Exception $e){
            Db::rollback();
            $this->error_new('操作失败');
        }
    ...
    public function betting(Request $request){
        ...
        $gm_name = $request->param('gm_name');
        if($gm_name){
            $uid = Db::name('member')->where('gm_name',$gm_name)->value('id');
            if($uid){
                $w['s.uid']  = $uid;
            }
            $w['m.gm_name']=$gm_name;
    ...
    public function win_lose(Request $request){
        ...
        $betting_total = Detail::where($w)
            ...
            ->sum('money');
        //总撤单
        $cedan_total = Detail::where($w)
            ->where(array('exp'=>3))
            ->sum('money');
        $betting_total = bcsub($betting_total,$cedan_total,2);
        //总中奖
    ...
    public function team_detail(Request $request){
        ...
        if(!empty($data['gm_name'])){
            $mem = Db::name('member')
                ->field('id,gm_name,rebate,bonus,proxy_ids,junior_ids,junior_num,money,is_proxy,create_at,back_rebate,update_at')
                ->where('gm_name',$data['gm_name'])
                ->find();
        }
        if(empty($mem)){
            $uids = Db::name('member')
        ...
        $mem['teamNum'] = count(explode(',',$mem['junior_ids']));
        //当前代理级别
        if(!isset($mem['is_proxy'])){
            $mem['is_proxy'] = '平台';
        }else{
            if($mem['is_proxy'] == 1){
                $mem['is_proxy'] = '总代理';
        ...
        if(!empty($data['from']) && !empty($data['to'])){
            $start = strtotime($data['from']);
            $end = strtotime($data['to'])+86400;
