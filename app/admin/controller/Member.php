...
//            $data['bonus'] = $rebateArr[1];
    ...
    public function del(Request $request){
        ...
        Db::startTrans();
        try{
            Db::name('single')->where('uid',$id)->delete();
            Db::name('detail')->where('uid',$id)->delete();
            Db::name('user_card')->where('uid',$id)->delete();
            Db::name('recharge')->where('uid',$id)->delete();
            Db::name('withdrawals')->where('uid',$id)->delete();
            Db::name('oauth_user')->where('uid',$id)->delete();
            Db::name('add_link')->where('uid',$id)->delete();
...
//            $data['bonus'] = $rebateArr[1];
           ...
           if(!is_numeric($data['back_rebate'])){
               $this->error_new('请填写正确返点');
           }
