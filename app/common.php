 ...
 * @param  integer $room_id 聊天室ID
 * @param  json    $content 内容
 * @return mix     $result
 */
function rongMsg($uid,$room_id,$content,$type="RC:TxtMsg"){
    $rong_key_secret=get_rong_key_secret();
    $RongCloud =new RongCloud($rong_key_secret['key'],$rong_key_secret['secret']);
    $result = $RongCloud->message()->publishChatroom($uid, $room_id,$type,$content);
    return $result;
}
...
function get_rongcloud_token($uid){
    // 从数据库中获取token
    $token=Db::name('oauth_user')->where('uid',$uid)->where('type',1)->value('access_token');
    // 如果有token就返回
    if ($token) {
        return $token;
    }
    // 获取用户昵称和头像
    $user_data=Db::name('member')->field('gm_name,head')->where('id',$uid)->find();
    // 用户不存在
    if (empty($user_data)) {
        return false;
    }
    if(empty($user_data['head'])){
        $user_data['head'] ="/static/home/img/wap_new/head.png";
    }
    // 获取头像url格式
    $avatar=get_url($user_data['head']);
    // 获取key和secret
    $key_secret=get_rong_key_secret();
    // 实例化融云
    $rong_cloud=new \Nx\RongCloud($key_secret['key'],$key_secret['secret']);
    // 获取token
    $token_json=$rong_cloud->getToken($uid,$user_data['gm_name'],$avatar);
    ...
    $data=array(
        'uid'=>$uid,
        'type'=>1,
        'nickname'=>$user_data['gm_name'],
        'head_img'=>$avatar,
        'access_token'=>$token
    );
    // 插入数据库
    $result=Db::name('oauth_user')->insert($data);
    if ($result) {
        return $token;
    }else{
        return false;
...
//记录明细
function addDetail($uid,$type,$money,$balance,$exp,$cate,$explain='',$hall=0,$single = 0 ,$is_room=0){
    $data=[
        'uid'=>$uid,
        'type'=>$type,
        'money'=>$money,
...
function getFanli2($userPl,$money){
    if($userPl['back_rebate'] == $userPl['proxy_rebate']){
        return 0;
    }else{
        $a  =bcsub( $userPl['proxy_rebate'],$userPl['back_rebate'],4);
        if($a>0){
            $bb = bcmul($money,$a,2);
        }else{
            $bb=0;
...
function setStageTime($cate_table){
    //计算下期的数字
    $whereData['end_time'] = array('gt',time());
    $a = Db::name($cate_table.'_stage')->where($whereData)->find();
    $endTime = $a['end_time']-time();
    $fenTime = $a['end_time']-$a['end_bet_time'];
    return ['stage_next'=>$a['stage'],'dateline'=>$endTime,'endBetTime'=>$a['end_bet_time'],'fenTime'=>$fenTime];
}
//计算（时间和期数）
...
/*------------香港六合彩end------------------*/
function getExp($id){
    $exp = Db::name('detail_exp')->where('id',$id)->value('name');
    return $exp;
}

//获取房间名称
function getHall($cate,$hall){
    $hallInfo = Db::name('hall')->where('cate',$cate)->where('hall',$hall)->cache(60)->find();
    if(isset($hallInfo['name'])){
        return $hallInfo['name'];
    }else{
        return '未知';
    }
}
function getCate($cate,$field){
    $hallInfo = Db::name('cate')->where('id',$cate)->cache(60)->find();
    if(isset($hallInfo[$field])){
        return $hallInfo[$field];
    }else{
        return '未知';
    }
}
