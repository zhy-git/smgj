    ...
    <a href="<?php echo url('Msssc/index',['cate'=>4,'hall'=>1]); ?>"  <?php if($controllerName == 'msssc'): ?>class="selected"<?php endif; ?>><span>极速时时彩</span> <i class="editbtn" style="display: none;"></i></a>
        ...
        <div class="header-bottom sub">
            <div class="show cate_menu">
                <a href="<?php echo url('index',['cate'=>8,'hall'=>1]); ?>" class="">特码</a> |
                ...
                <a href="<?php echo url('index',['cate'=>8,'hall'=>1,'type'=>'eighteen']); ?>" class="router-link-exact-active selected">连尾</a> |
                ...
                <a href="<?php echo url('index',['cate'=>8,'hall'=>1,'type'=>'three']); ?>" class="">总肖、波色</a> |
                    ...
                    <div class="cont-col3">
                        <div class="u-header play_tab_1 clearfix">
                            <div class="fl">
                                ...
                                <span id="page_name">连肖</span> <span id="total_sum_money_text">
                                &nbsp;&nbsp;
                                当前彩种输赢：<span id="total_sum_money">0.000</span></span></div>
                            <div class="fr"><span id="next_turn_num">2018006</span>&nbsp;期 距离封盘：
                                <span id="bet-date">1天 11:03:03</span> 距离开奖：
                                <span id="open-date">1天 11:08:03</span></div></div>
                        <div><div class="cont-col3-hd clearfix">
                            <div class="cont-col3-box2"><span>金额 </span>
                                <input type="number" class="bet-money" value="">
                                ...
                                <div>
                                    <div class="cont-col3-bd">
                                        <ul class="tab" style="height: 35px;">
                                            <li class="tab_title02" id="changeTitle">
                                                <?php if(is_array($rule_data['type']) || $rule_data['type'] instanceof \think\Collection || $rule_data['type'] instanceof \think\Paginator): $k = 0; $__LIST__ = $rule_data['type'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
                                                <a href="javascript:void(0)" class="<?php echo !empty($k) && $k==1?'on':''; ?>" data-id="<?php echo $vo['type']; ?>" data-num="<?php echo $vo['title']; ?>"><?php echo $vo['title']; ?></a>
                                                <?php endforeach; endif; else: echo "" ;endif; ?>
                                            </li>
                                        </ul>
                                        <table class="u-table2" id="changeTab">
                                            <thead>
                                            <tr>
                                                <th>尾号</th>
                                                <th>号</th>
                                            ...
                                            <?php if(is_array($rule_data['lw']) || $rule_data['lw'] instanceof \think\Collection || $rule_data['lw'] instanceof \think\Paginator): $k = 0; $__LIST__ = $rule_data['lw'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?>
                                            <tbody id="box_<?php echo $v['type']; ?>" style="display:<?php echo $k==1?'inline_block':'none'; ?>">
                                            ...
                                            <tr data-target-type="<?php echo $v['type']; ?>" data-target-num="<?php echo $v['type_name']; ?>">
                                                ...
                                                <td data-id="<?php echo $v1['id']; ?>" data-num="<?php echo $v1['rule']; ?>" class="">
                                                    <div class="qiu">
                                                        ...
                                                        <i class="lhc_icon lhc_b<?php echo $v2; ?> mr2"><?php echo $v2; ?></i>
                                                        <?php endforeach; endif; else: echo "" ;endif; ?>
                                                    </div>
                                                </td>
                                                <td class="odds not-event">
                                                    <span class="c-txt3"><?php echo $v1['rate']; ?></span>
                                                </td>
                                                <td data-id="<?php echo $v1['id']; ?>" data-num="<?php echo $v1['rule']; ?>" data-odds="<?php echo $v1['rate']; ?>" class="checkbox_td">
                                                    <input type="checkbox">
                                                </td>
                                                <?php endforeach; endif; else: echo "" ;endif; ?>
                                            </tr>
                            ...
                            <div class="cont-col3-hd clearfix" style="margin-top: 15px;">
                                <div class="cont-col3-box2">
                                    <span>金额 </span>
                                    <input type="number" class="bet-money" value="1">
                                    <a href="javascript:void(0)" class="u-btn1 betTab">确定</a>
                                    <a href="javascript:void(0)" class="u-btn1 betReset">重置</a>
                                </div>
                            </div>
...
<script type="text/javascript">
    $(function(){
        $(".tab_title02 a").click(function(){
            $(this).addClass("on").siblings().removeClass("on");
            $(".checkbox_td input").prop('checked',false);
            var dataId = $(this).attr('data-id');
            $('#box_'+dataId).siblings('tbody').css('display','none');
            $('#box_'+dataId).show();
            lotteryBetController.oddsSet();
            //玩法颜色变化
            $(".cont-main table .name").click(function(){
                if($(this).next().find('span').html()=='--'){
                    return;
                }
                if($(this).hasClass("bg_yellow")){
                    $(this).removeClass("bg_yellow");
                    $(this).next().removeClass("bg_yellow");
                    $(this).next().next().removeClass("bg_yellow");
                    $(this).next().next().children('input').val('');
                }else{
                    $(this).addClass("bg_yellow");
                    $(this).next().addClass("bg_yellow");
                    $(this).next().next().addClass("bg_yellow");
                    if($('.bet-money').val()){
                        $(this).next().next().children('input').val($('.bet-money').val());
                    }else{
                        $(this).next().next().children('input').val(0);
                    }
                }
            });
            $(".cont-main table .odds").click(function(){
                if($(this).find('span').html()=='--'){
                    return;
                }
                if($(this).hasClass("bg_yellow")){
                    $(this).removeClass("bg_yellow");
                    $(this).prev().removeClass("bg_yellow");
                    $(this).next().removeClass("bg_yellow");
                    $(this).next().children('input').val('');
                }else{
                    $(this).addClass("bg_yellow");
                    $(this).prev().addClass("bg_yellow");
                    $(this).next().addClass("bg_yellow");
                    if($('.bet-money').val()){
                        $(this).next().children('input').val($('.bet-money').val());
                    }else{
                        $(this).next().children('input').val(0);
                    }
                }
            });
            $(".cont-main table .amount input").focus(function(){
                if($(this).parent().prev().find('span').html()=='--'){
                    return;
                }
                if($(this).hasClass("bg_yellow")){
                    $(this).parent().removeClass("bg_yellow");
                    $(this).parent().prev().removeClass("bg_yellow");
                    $(this).parent().prev().prev().removeClass("bg_yellow");
                    $(this).val('');
                }else{
                    $(this).parent().addClass("bg_yellow");
                    $(this).parent().prev().addClass("bg_yellow");
                    $(this).parent().prev().prev().addClass("bg_yellow");
                    if($('.bet-money').val()){
                        $(this).val($('.bet-money').val());
                    }else{
                        $(this).val(0);
                    }
                }
            });
            $(".cont-main table .name").hover(function(){
                $(this).addClass("hover");
            ...
            $(".cont-main table .odds").hover(function(){
                $(this).addClass("hover");
            ...
            $(".cont-main table .amount").hover(function(){
                $(this).addClass("hover");
                $(this).prev().addClass("hover");
